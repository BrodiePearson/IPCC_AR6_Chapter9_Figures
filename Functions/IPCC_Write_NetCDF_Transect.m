function  IPCC_Write_NetCDF_Transect(ncfilename, var_name, var_units, var, ...
    latitude, depth, title, comments, hatching_mask)
%IPCC_Write_NetCDF_Transect creates a NetCDF file containing IPCC Transect data
%
%   IPCC_Write_NetCDF_Transect(ncfilename, var_name, var_units, var, ...
%       latitude, depth, title, comments, hatching_mask)
%
% Input variables:
...
nccreate(ncfilename,var_name,'Dimensions', ...
    {'Depth',size(depth,1), 'Latitude',size(latitude,1)});
...
nccreate(ncfilename,'Depth', ...
    'Dimensions', {'Depth',size(depth,1)});
...
ncwrite(ncfilename,'Depth',depth);
