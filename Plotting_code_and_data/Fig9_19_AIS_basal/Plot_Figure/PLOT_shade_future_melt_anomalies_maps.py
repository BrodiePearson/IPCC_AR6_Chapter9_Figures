...
# December 2019
#
# Used to plot maps of future change in Antarctic melt rates
# (i.e. anomaly with respect to present-day).
#
...
filevars='vars_future_anom_'+what+'.npz' # files in which variables have been saved (to be used by AR6 team)
...
if os.path.exists(filevars):
  ...
  if ( what == 'FESOM_PRES' ):
    ...
    file_melt = homedir+'/XYLAR_ISMIP6_DATA/IMBIE2/melt_rates_'+what+'_noresm1-m_rcp8.5.nc'
  ...
  # melt anomaly = (2081-2100) - present-day (i.e. 1995-2014 for CMIP5 and 2006-2015 for FESOM)
  if ( what == 'FESOM_PRES' ):
    melt_anom = np.mean( ncM['melt'].values[76:96,:,:], axis=0 ) - np.mean( ncM['melt'].values[0:10,:,:], axis=0 )
  else:
    melt_anom = np.mean( ncM['melt_rate'].values[87:107,:,:]*1.e0, axis=0 ) - np.mean( ncM['melt_rate'].values[0:20,:,:]*1.e0, axis=0 )
  melt2=melt_anom[:,:]*1.e0
  melt2[ melt2 == 0.e0 ] = np.nan 
  print('max melt anom = ', np.nanmax(melt2))
  print('min melt anom = ', np.nanmin(melt2))
  ...
  icesheet_msk[ icesheet_msk > 1.5 ]= 1.e0
...
cax=ax.contourf(x*1.e-3,y*1.e-3,melt2,levs,cmap=newcmp,norm=matplotlib.colors.SymLogNorm(linthresh=lin_threshold, linscale=lin_scale,vmin=minval, vmax=maxval))
ax.contour(x*1.e-3,y*1.e-3,basnb,np.linspace(0.5,20.5,21),linewidths=0.5,colors='gray')
ax.contour(x*1.e-3,y*1.e-3,grounded_msk,linewidths=0.5,colors='black')
ax.contour(x*1.e-3,y*1.e-3,icesheet_msk,linewidths=0.5,colors='black')
# Zoom on Amundsen:
zoomfac=2.85
xll_ori = -2000.0
yll_ori =  -900.0
...
if ( what == 'NON_LOCAL' ):
  title="Changes until 2081-2100 (ISMIP6 non-local-MeanAnt)"
elif ( what == 'NON_LOCAL_MAX_PIG' ):
  title="Changes until 2081-2100 (ISMIP6 non-local-PIGL)"
elif ( what == 'FESOM_PRES' ):
  title="Changes until 2081-2100 (FESOM-MMM-rcp85)"
else:
  title=what
...
namefig='../PNGs/map_melt_AR6_future_anom_'+what+'.pdf'  
fig.savefig(namefig)
